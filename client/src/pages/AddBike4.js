...
export const AddBike4 = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const response = await axios.post('http://localhost:4000/api/bikes', formData, {
        headers: {
        ...
        >
          <Grid item md={12}>
            <Typography
              ...
              pb={2}
              color="#154B46"
            > Add Photos</Typography>
          </Grid>
          <Grid item md={12}>
            <form onSubmit={handleSubmit} encType="multipart/form-data">
              <FormControl >
                {/* <InputLabel htmlFor="images-input">Images:</InputLabel> */}
                <Input id="images-input" type="file" multiple onChange={handleImageChange} inputProps={{ multiple: true }}/>
              </FormControl>
              <Button type="submit" variant="contained" onClick={handleSubmit}>Submit</Button>
              <Grid item md={12} pt={1}>
                {selectedFiles.length > 0 && (<><Typography
                  variant="h4"
                  sx={{
                    fontWeight: "bold",
                  }}
